  ...
  if session.get('logged_in'):
    if session['usertype'] == 0 or session['usertype'] == 1:
      try:
        ...
        add_log = open(ACTIVITY_LOG, "a")
        add_log.write(
          "%s - %s unsucces restart event %s node's %s process .\n" % (
          datetime.now().ctime(), session['username'], node_name, process_name))
        return JsonValue(process_name, node_name, "restart").error(
    ...
    add_log = open(ACTIVITY_LOG, "a")
    add_log.write(
      "%s - Illegal request for restart to %s node's %s process %s .\n" % (
  ...
  if session.get('logged_in'):
    if session['usertype'] == 0 or session['usertype'] == 1:
      try:
        ...
        add_log = open(ACTIVITY_LOG, "a")
        add_log.write(
          "%s - %s unsucces start event %s node's %s process .\n" % (
          datetime.now().ctime(), session['username'], node_name, process_name))
        return JsonValue(process_name, node_name, "start").error(err.faultCode,
                                                                 err.faultString)
    else:
      add_log = open(ACTIVITY_LOG, "a")
      add_log.write(
    ...
    add_log = open(ACTIVITY_LOG, "a")
    add_log.write(
      "%s - Illegal request for start to %s node's %s process %s .\n" % (
      datetime.now().ctime(), node_name, process_name))
    return redirect(url_for('login'))
  ...
  if session.get('logged_in'):
    if session['usertype'] == 0 or session['usertype'] == 1:
      try:
        ...
        datetime.now().ctime(), session['username'], node_name, process_name))
        return JsonValue(process_name, node_name, "stop").error(err.faultCode,
                                                                err.faultString)
    else:
      add_log = open(ACTIVITY_LOG, "a")
      add_log.write(
    ...
    add_log = open(ACTIVITY_LOG, "a")
    add_log.write(
      "%s - Illegal request for stop to %s node's %s process %s .\n" % (
      datetime.now().ctime(), node_name, process_name))
    return redirect(url_for('login'))
...
def readlog(node_name, process_name):
  if session.get('logged_in'):
    if session['usertype'] == 0 or session['usertype'] == 1 or session[
      'usertype'] == 2:
      node_config = Config(CONFIG_FILE).getNodeConfig(node_name)
      node = Node(node_config)
      log = \
      node.connection.supervisor.tailProcessStdoutLog(process_name, 0, 500)[0]
      add_log = open(ACTIVITY_LOG, "a")
      add_log.write("%s - %s read log %s node's %s process .\n" % (
      datetime.now().ctime(), session['username'], node_name, process_name))
      return jsonify(status="success",
                     url="node/" + node_name + "/process/" + process_name + "/read",
                     log=log)
    else:
      add_log = open(ACTIVITY_LOG, "a")
      add_log.write(
    ...
    add_log = open(ACTIVITY_LOG, "a")
    add_log.write(
      "%s - Illegal request for read log to %s node's %s process %s .\n" % (
      datetime.now().ctime(), node_name, process_name))
...
def del_user():
  if session.get('logged_in'):
    if session['usertype'] == 0:
      ...
      add_log = open(ACTIVITY_LOG, "a")
      add_log.write(
        "%s - Unauthorized user request for delete user event. Delete user event fail .\n" % (
        datetime.now().ctime()))
      return jsonify(status='error')
  ...
  if session.get('logged_in'):
    if session['usertype'] == 0:
      if username != "admin":
        ...
        add_log = open(ACTIVITY_LOG, "a")
        add_log.write(
          "%s - %s  user request for delete admin user. Delete admin user event fail .\n" % (
          datetime.now().ctime(), session['username']))
        return jsonify(status="error",
      ...
      add_log = open(ACTIVITY_LOG, "a")
      add_log.write(
        "%s - %s is unauthorized user for request to delete a user. Delete event fail .\n" % (
        datetime.now().ctime(), session['username']))
      return jsonify(status="error",
...
def adduserhandler():
  if session.get('logged_in'):
    if session['usertype'] == 0:
      ...
      add_log = open(ACTIVITY_LOG, "a")
      add_log.write(
        "%s - %s is unauthorized user for request to add user event. Add user event fail .\n" % (
        datetime.now().ctime(), session['username']))
      return jsonify(status="error",
    ...
    add_log = open(ACTIVITY_LOG, "a")
    add_log.write(
      "%s - Illegal request for add user event.\n" % (datetime.now().ctime()))
    return jsonify(status="error",
...
def changepassword(username):
  if session.get('logged_in'):
    if session['username'] == username:
      ...
      add_log = open(ACTIVITY_LOG, "a")
      add_log.write(
        "%s - %s user request to change %s 's password. Change password event fail\n" % (
