...
class Config:
  ...
  def getNodeConfig(self, node_name):
    ...
    self.node_config = NodeConfig(self.node_name,
                                  self.host,
                                  self.port,
                                  self.username,
                                  self.password)
...
class Node:
  def __init__(self, node_config):
    ...
    self.connection = Connection(node_config.host,
                                 node_config.port,
...
class Connection:
  def __init__(self, host, port, username, password):
    ...
    self.username = username
    self.password = password
    self.address = "http://%s:%s@%s:%s/RPC2" % (self.username,
                                                self.password,
                                                self.host,
                                                self.port)
...
class ProcessInfo:
  def __init__(self, dictionary):
    ...
    self.start_hr = datetime.fromtimestamp(
        self.dictionary['start']).strftime('%Y-%m-%d %H:%M:%S')[11:]
    self.stop_hr = datetime.fromtimestamp(
        self.dictionary['stop']).strftime('%Y-%m-%d %H:%M:%S')[11:]
    self.now_hr = datetime.fromtimestamp(
        self.dictionary['now']).strftime('%Y-%m-%d %H:%M:%S')[11:]
...
class JsonValue:
  ...
  def success(self):
    return jsonify(status="Success",
                   code=80,
                   message="%s %s %s event succesfully" % (self.node_name,
                                                           self.process_name,
                                                           self.event),
                   nodename=self.node_name,
                   data=self.node.connection.supervisor.getProcessInfo(
                       self.process_name))
  ...
  def error(self, code, payload):
    ...
    return jsonify(status="Error",
                   code=self.code,
                   message="%s %s %s event unsuccesful" % (self.node_name,
                                                           self.process_name,
                                                           self.event),
                   nodename=self.node_name,
